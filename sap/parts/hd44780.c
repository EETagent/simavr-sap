  ...
  if (hd44780_get_flag(b, HD44780_FLAG_I_D)) { // incrementing
    if (b->cursor < 0x80) {                    // cursor in DDRAM
      ...
      if (hd44780_get_flag(b, HD44780_FLAG_N)) { // 2-line display
        if (b->cursor >= 0x00 + 40 &&
            b->cursor < 0x40) // jump from end of first memory segment to the
                              // start of the second segment
          b->cursor = 0x40;
        else if (b->cursor >=
                 0x40 + 40) // wrap around from the end of the second memory
                            // segment to the start of the first segment
          b->cursor = 0x00;
      } else { // 1-line display
        if (b->cursor >=
            0x00 + 80) // wrap around from the end of the memory to the start
          b->cursor = 0x00;
      }
    ...
    if (b->cursor < 0x80) {                      // cursor in DDRAM
      if (hd44780_get_flag(b, HD44780_FLAG_N)) { // 2-line display
        if (b->cursor == 0x40) // fall back from the start of the second memory
                               // segment to the end of the first segment
          b->cursor = 0x00 + 39;
        else if (b->cursor ==
                 0x00) // wrap around from the start of the first memory segment
                       // to the end of the second segment
          b->cursor = 0x40 + 39;
        else
          b->cursor--;
      } else { // 1-line display
        if (b->cursor ==
            0x00) // wrap around from the start of the memory to the end
          b->cursor = 0x00 + 79;
        else
          b->cursor--;
      }
...
static uint32_t hd44780_write_command(hd44780_t *b) {
  ...
  case 7: // 1 ADD ADD ADD ADD ADD ADD ADD
    ...
    if (hd44780_get_flag(b, HD44780_FLAG_N)) { // 2-line display
      if (b->cursor >= 0x00 + 40 &&
          b->cursor < 0x40) // illegal address after the first memory segment ->
                            // set cursor to start of second segment
        b->cursor = 0x40;
      else if (b->cursor >=
               0x40 + 40) // illegal address after the second memory segment ->
                          // set cursor to start of first segment
        b->cursor = 0x00;
    } else { // 1-line display
      if (b->cursor >=
          0x00 +
              80) // illegal address after valid memory -> set cursor to start
        b->cursor = 0x00;
    }
    break;
  // Set CGRAM address
...
static uint32_t hd44780_process_write(hd44780_t *b) {
  ...
  if (four) { // 4 bits !
    if (comp)
      b->datapins =
          (b->datapins & 0xf0) | ((b->pinstate >> IRQ_HD44780_D4) & 0xf);
    else
      b->datapins =
          (b->datapins & 0xf) | ((b->pinstate >> (IRQ_HD44780_D4 - 4)) & 0xf0);
    write = comp;
...
static avr_cycle_count_t _hd44780_process_e_pinchange(struct avr_t *avr,
  ...
  if (delay) {
    ...
    avr_cycle_timer_register_usec(b->avr, delay, _hd44780_busy_timer, b);
  }
...
static void hd44780_pin_changed_hook(struct avr_irq_t *irq, uint32_t value,
                                     void *param) {
  hd44780_t *b = (hd44780_t *)param;
  ...
  case IRQ_HD44780_ALL:
    for (int i = 0; i < 4; i++)
      hd44780_pin_changed_hook(b->irq + IRQ_HD44780_D4 + i, ((value >> i) & 1),
                               param);
...
void hd44780_init(struct avr_t *avr, struct hd44780_t *b, int width,
                  int height) {
  memset(b, 0, sizeof(*b));
  b->avr = avr;
  b->w = width;

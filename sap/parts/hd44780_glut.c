...
void hd44780_gl_init()
{
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, 4, lcd_font.width, lcd_font.height, 0, GL_RGBA,
               GL_UNSIGNED_BYTE, lcd_font.pixel_data);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);

  glMatrixMode(GL_TEXTURE);
  glLoadIdentity();
  glScalef(1.0f / (GLfloat)lcd_font.width, 1.0f / (GLfloat)lcd_font.height,
           1.0f);
}

static inline void glColor32U(uint32_t color)
{
...
void hd44780_gl_draw(hd44780_t *b, uint32_t background, uint32_t character,
                     uint32_t text, uint32_t shadow)
{
  int rows = b->w;
  int lines = b->h;
  int border = 3;
  ...
  glVertex3f(rows * charwidth + (rows - 1) + border,
             lines * charheight + (lines - 1) + border, 0);
